...
/**
 * End-to-end WireMock'd LLM pipeline tests: PASS, REVISE→PASS,
 * REVISE→REVISE→BLOCK, upstream timeout, malformed compliance output.
 * Every scenario must leave an audit record.
 */
@QuarkusTest
...
class ExplainFlowTest {
    ...
    void happyPathPassesOnFirstDraft() {
        ...
        String auditId = given()
                ...
                .body("{\"question\":\"Why is the Westpac Active Growth fund down this year?\",\"audience\":\"INVESTOR\"}")
                .when().post("/api/v1/explain")
                .then().statusCode(200)
    ...
    void reviseThenPassRetriesWriterOnce() {
        ...
        server.stubFor(chatFor(COMPLIANCE_MARKER).inScenario("compliance")
                .whenScenarioStateIs(STARTED)
                .willReturn(okJson(chatBody(complianceJson("REVISE", "Missing citation for the -4.1% figure"))))
        ...
        String auditId = given()
                .contentType(ContentType.JSON)
                .body("{\"question\":\"Explain the Westpac fund's recent performance\",\"audience\":\"ADVISER\"}")
                .when().post("/api/v1/explain")
                .then().statusCode(200)
                .body("complianceVerdict", equalTo("PASS"))
                .extract().path("auditId");
    ...
    void doubleReviseBlocksWithFallbackAnswer() {
        LlmStubs.stubAgent(COMPLIANCE_MARKER, complianceJson("REVISE", "Still contains advice phrasing"));

        String auditId = given()
                .contentType(ContentType.JSON)
                .body("{\"question\":\"Should I switch to Simplicity Growth?\",\"audience\":\"INVESTOR\"}")
                .when().post("/api/v1/explain")
                .then().statusCode(200)
    ...
    void ollamaTimeoutReturns503ProblemDetailAndStillAudits() {
        WireMockTestResource.server().stubFor(chatFor(RESEARCH_MARKER)
                .atPriority(1)
                .willReturn(aResponse().withFixedDelay(8000).withBody(chatBody("too late"))
                        .withHeader("Content-Type", "application/json")));

        String auditId = given()
                .contentType(ContentType.JSON)
                .body("{\"question\":\"Compare fees across all growth funds\",\"audience\":\"ADVISER\"}")
                .when().post("/api/v1/explain")
                .then().statusCode(503)
