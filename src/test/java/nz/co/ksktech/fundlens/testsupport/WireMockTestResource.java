...
/**
 * One WireMock server for everything external: the Ollama API (chat +
 * embeddings), the Disclose Register gateway (under /disclose) and the
 * "CDN" serving redirected fund-update documents (under /files).
 * No test ever talks to the network.
 */
public class WireMockTestResource implements QuarkusTestResourceLifecycleManager {
    ...
    private static void registerDefaultStubs() {
        ...
        server.stubFor(post(urlEqualTo("/api/embed")).atPriority(10)
                .willReturn(aResponse().withTransformers("embed-transformer")));
        // Legacy single-prompt endpoint.
        server.stubFor(post(urlEqualTo("/api/embeddings")).atPriority(10)
    ...
    public static class EmbedTransformer implements ResponseDefinitionTransformerV2 {
        ...
        public ResponseDefinition transform(ServeEvent serveEvent) {
            ...
            String body = "{\"model\":\"nomic-embed-text\",\"embeddings\":["
