...
/**
 * Helpers for stubbing the Ollama /api/chat endpoint. The three agents share the endpoint; requests
 * are told apart by distinctive phrases from their system prompts.
 */
public final class LlmStubs {
  ...
  public static void stubAgent(String agentMarker, String responseContent) {
    WireMockTestResource.server()
        .stubFor(chatFor(agentMarker).willReturn(okJson(chatBody(responseContent))));
  }
