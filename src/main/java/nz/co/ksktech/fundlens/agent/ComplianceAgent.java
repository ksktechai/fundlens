...
/**
 * Adversarial reviewer; runs on its own named model so production can point it at a different
 * (stronger) model than the writer. Structured output: the framework parses the JSON verdict into
 * {@link ComplianceResult}; parse failures are mapped to BLOCK by ComplianceGate.
 */
@RegisterAiService(
