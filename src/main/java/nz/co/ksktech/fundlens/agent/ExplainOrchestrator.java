...
/**
 * research → write → compliance pipeline with one revise loop. Every request
 * — answered, blocked or failed upstream — leaves an append-only AuditRecord.
 */
@ApplicationScoped
public class ExplainOrchestrator {
    ...
    public ExplainOrchestrator(ResearchAgent researchAgent,
    ...
    public Outcome explain(String question, List<Long> fundIds, Audience audience) {
        ...
        try {
            ...
            if (result.verdict() == Verdict.REVISE) {
                draft = writerAgent.write(question, audience.name(), findings,
            ...
            UUID auditId = appendAudit(question, scope, audience, findings, drafts, complianceResults,
                    answer, passed ? "ANSWERED" : "BLOCKED", startNanos);
            return new Outcome(auditId, answer, finalVerdict, citations());
        } catch (RuntimeException e) {
            Log.errorf(e, "Explain pipeline failed for question: %s", question);
            UUID auditId = appendAudit(question, scope, audience, findings, drafts, complianceResults,
                    null, "UPSTREAM_ERROR", startNanos);
            throw new LlmUnavailableException(auditId, e);
        }
    }

    private UUID appendAudit(String question, List<Long> fundIds, Audience audience, String findings,
                             List<String> drafts, List<ComplianceResult> complianceResults,
                             String finalAnswer, String status, long startNanos) {
