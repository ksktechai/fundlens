...
/**
 * Plain HTTP download of the redirect target of a fund-update-document call. Deliberately NOT the
 * REST client: the Location URL points outside the API gateway and must be fetched without the
 * Ocp-Apim-Subscription-Key or x-organisation headers.
 */
@ApplicationScoped
    ...
    try {
      ...
      if (response.statusCode() != 200) {
        throw new DiscloseApiException(
            response.statusCode(), "document download failed for " + url);
      }
      Log.infof(
          "<<< %d %s (%d bytes, %s)",
          response.statusCode(),
      ...
      throw new DiscloseApiException(
          0, "document download failed for " + url + ": " + e.getMessage());
