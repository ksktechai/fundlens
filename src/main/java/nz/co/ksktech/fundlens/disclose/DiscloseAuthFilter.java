...
/**
 * Adds the API-gateway subscription key to every Disclose Register request.
 * Registered on {@link DiscloseRegisterClient} via {@code @RegisterProvider}.
 * The key must never travel to redirect targets — the REST client is
 * configured with follow-redirects=false and document downloads go through
 * {@link DocumentFetcher} without this filter.
 */
