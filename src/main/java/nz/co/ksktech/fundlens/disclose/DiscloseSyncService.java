...
/**
 * Nightly (and manually triggered) ETag-based incremental sync from the
 * Disclose Register. One fund failing never aborts the rest; every run is
 * recorded as a {@link SyncRun} row with per-fund outcomes.
 */
@ApplicationScoped
public class DiscloseSyncService {

    public record FundOutcome(String fundNumber, String status, String message, boolean documentIngested) {
    }

    public record SyncSummary(Long runId, Instant startedAt, Instant finishedAt,
                              String status, List<FundOutcome> outcomes) {
    }
    ...
    public DiscloseSyncService(DiscloseService discloseService,
    ...
    private FundOutcome syncFund(String fundNumber) {
        String storedEtag = QuarkusTransaction.requiringNew().call(
                () -> Fund.findByDiscloseFundNumber(fundNumber).map(f -> f.discloseEtag).orElse(null));
        ...
        try {
            ...
            ingestionService.ingest(document, fund.fundName() + " - fund update.pdf",
            ...
            return new FundOutcome(fundNumber, "UPDATED", "document ingestion failed: " + e.getMessage(), false);
        }
    }

    private Long upsert(String fundNumber, FundFetchResult result) {
        DiscloseFund source = result.fund();
        Fund fund = Fund.findByDiscloseFundNumber(fundNumber).orElseGet(() -> {
        ...
        FundMetrics metrics = FundMetrics.findByFundAndPeriod(fund.id, source.reportingPeriodEndDate())
                .orElseGet(() -> {
    ...
    private Long persistRun(Instant started, Instant finished, String trigger,
                            String status, List<FundOutcome> outcomes) {
    ...
    private static String providerOf(DiscloseFund fund) {
        return fund.offerNumber() != null ? "Disclose offer " + fund.offerNumber() : "Disclose Register";
    }
