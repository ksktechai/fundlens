...
/**
 * Logs every API request and response, including JSON/text bodies (truncated).
 * Binary payloads (multipart uploads, PDFs) are logged by size only.
 * Toggle with fundlens.log.http-bodies / LOG_HTTP_BODIES; the one-line
 * access log (quarkus.http.access-log) stays on independently.
 */
@Provider
public class HttpLoggingFilter implements ContainerRequestFilter, ContainerResponseFilter, WriterInterceptor {
    ...
    private static String truncate(String body) {
        ...
        return single.substring(0, BODY_LOG_LIMIT) + "…(+" + (single.length() - BODY_LOG_LIMIT) + " chars)";
    }
}
