...
public class ProblemMappers {
    ...
    public record Problem(String type, String title, int status, String detail,
                          List<String> violations, UUID auditId) {
    ...
    public RestResponse<Problem> constraintViolation(ConstraintViolationException e) {
        List<String> violations = e.getConstraintViolations().stream()
                ...
                .sorted()
                .toList();
        Problem problem = new Problem("about:blank", "Validation failed", 400,
                "Request validation failed", violations, null);
    ...
    public RestResponse<Problem> llmUnavailable(LlmUnavailableException e) {
        ...
        Problem problem = new Problem("about:blank", "LLM backend unavailable", 503,
    ...
    public RestResponse<Problem> fallback(Exception e) {
        if (e instanceof WebApplicationException wae) {
            ...
            return problemResponse(Problem.of(
        ...
        return problemResponse(Problem.of("Internal server error", 500,
                "An unexpected error occurred"));
    }

    private static RestResponse<Problem> problemResponse(Problem problem) {
        return RestResponse.ResponseBuilder
                .create(RestResponse.Status.fromStatusCode(problem.status()), problem)
