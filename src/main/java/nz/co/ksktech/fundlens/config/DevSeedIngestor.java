...
/**
 * Dev-mode only: ingests the sample fund updates under src/main/resources/seed
 * at startup so RAG answers work without any external setup.
 */
@ApplicationScoped
public class DevSeedIngestor {
    ...
    void onStart(@Observes StartupEvent event) {
        ...
        try {
            ...
            Log.warnf("Dev seed ingestion skipped: %s (is the embedding model pulled on the Ollama host? "
                    + "ollama pull nomic-embed-text)", e.getMessage());
        }
    }
    ...
    private void ingestSeed(String resourcePath, String fundName) {
        try (InputStream in = Thread.currentThread().getContextClassLoader().getResourceAsStream(resourcePath)) {
            ...
            Long fundId = Fund.<Fund>find("name", fundName).firstResultOptional().map(f -> f.id).orElse(null);
            String provider = fundName.split(" ")[0];
            ingestionService.ingest(data, resourcePath,
                    new IngestionService.IngestionRequest(fundId, provider, "FUND_UPDATE",
                            LocalDate.of(2026, 3, 31),
                            fundName + " fund update (Mar 2026)", "DEV_SEED"));
