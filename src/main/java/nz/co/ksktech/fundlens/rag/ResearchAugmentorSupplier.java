...
/**
 * Retrieval augmentor for the ResearchAgent only (Writer and Compliance run
 * without RAG). Filters by fund_id metadata when the request scopes funds,
 * and records every retrieved chunk into {@link RetrievalContext} for the
 * audit trail.
 */
@ApplicationScoped
public class ResearchAugmentorSupplier implements Supplier<RetrievalAugmentor> {
    ...
    public ResearchAugmentorSupplier(EmbeddingStore<TextSegment> embeddingStore,
    ...
    private record TrackingRetriever(ContentRetriever delegate, RetrievalContext context)
        ...
        public List<Content> retrieve(Query query) {
            ...
            for (Content content : contents) {
                String embeddingId = String.valueOf(
                        content.metadata().getOrDefault(ContentMetadata.EMBEDDING_ID, ""));
                Object score = content.metadata().getOrDefault(ContentMetadata.SCORE, 0.0d);
                var segmentMetadata = content.textSegment().metadata();
                context.recordRetrieved(new RetrievalContext.RetrievedChunk(
                        embeddingId,
